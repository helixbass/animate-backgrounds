...
(function() {
  var Color, Tween, _int, extend, gradient_handler, map, register_animation_handler, type;
  ...
  gradient_handler = function(arg) {
    var function_name, hook_name;
    function_name = arg.function_name, hook_name = arg.hook_name;
    return {
      hook_name: hook_name,
      ...
      parse: function(val) {
        ...
        for (j = 0, len = ref.length; j < len; j++) {
          image = ref[j];
          results.push((function() {
            var all, angle, first_direction, match, second_direction, stops;
            match = RegExp("^\\s*" + function_name + "\\(\\s*(?:(?:(-?\\d+)deg|to\\s+(bottom|top|left|right)(?:\\s+(bottom|top|left|right))?)\\s*,\\s*)?(.+)\\)\\s*$").exec(image);
            if (!match) {
              return image;
            }
            all = match[0], angle = match[1], first_direction = match[2], second_direction = match[3], stops = match[4];
            return {
              angle: (function() {
                if (angle) {
                  return _int(angle);
              ...
              stops: (function() {
                var color, k, len1, position, results1, split_stops, stop, stop_index, unit;
        ...
        return ((function() {
          ...
          for (image_index = j = 0, len = start.length; j < len; image_index = ++j) {
            ...
            results.push((function() {
              ...
              return function_name + "(" + (_scaled(function(arg1) {
                var angle;
                angle = arg1.angle;
                return angle;
              })) + "deg, " + (((function() {
